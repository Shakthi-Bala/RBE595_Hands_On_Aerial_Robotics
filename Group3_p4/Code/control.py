...
class quad_control:
    def __init__(self):
        ...
        # self.x_pid = pid(1.5, 0.005, 0.68, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(0.95, 0.005, 0.68, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.z_pid = pid(5.0, 0.5,0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)

        # ########################## TODO - SET GAINS HERE ##############################
        # ###############################################################################
        # # NED velocity controller. EDIT GAINS HERE
        # self.vx_pid = pid(2.0, 0.2, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(1.3, 0.1, 1.4, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vz_pid = pid(25.0, 7.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
        ##############################################################################
        ##############################################################################

        self.x_pid = pid(0.725, 0.0, 0.68, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.y_pid = pid(0.2, 0.0, 0.78, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.z_pid = pid(2.3, 0.01,3.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)

        ########################## TODO - SET GAINS HERE ##############################
        ###############################################################################
        # NED velocity controller. EDIT GAINS HERE
        self.vx_pid = pid(2.8, 0.2, 0.3, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vy_pid = pid(0.7, 0.1, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vz_pid = pid(15.0, 4.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
