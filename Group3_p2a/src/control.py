...
class quad_control:
    def __init__(self):
        ...
        # self.x_pid = pid(3.5, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(3.5, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.z_pid = pid(1.5, 0.01,0.2, filter_tau, dt, minVal = minVel, maxVal=maxVel) # works ok
        # self.z_pid = pid(10, 0.0,4, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        ...
        # self.vx_pid = pid(2, 0, 0.1 , filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(5, 0, 0.4, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # # self.vz_pid = pid(10, 5, 0.0, filter_tau, dt, minVal = minAcc, maxVal = maxAcc) #is correct 
        # self.vz_pid = pid(1, 0, 0.0, filter_tau, dt, minVal = minAcc, maxVal = maxAcc) #test
        # # self.vz_pid= pid(-10, -5, 0.0, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)


        # self.x_pid = pid(2, 0.0, 0.5, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(3, 0.0, 0.5, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # # # self.z_pid = pid(1.5, 0.01,0.2, filter_tau, dt, minVal = minVel, maxVal=maxVel) # works ok
        # self.z_pid = pid(2, 0.0,0.5, filter_tau, dt, minVal = minVel, maxVal=maxVel) # works ok
        # self.z_pid = pid(-2, 0.0,-0.5, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        #################################################################################################################

        # test 3 set
        # self.x_pid = pid(2, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(2, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.z_pid = pid(1, 0.0,0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        
        # self.vx_pid = pid(10, 0.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(10, 0.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vz_pid = pid(20, 0, 0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
        ...
        # self.z_pid = pid(0.1,0.1,0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        
        # self.vx_pid = pid(2,0.5,0.5, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(2,0.5,0.5, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vz_pid = pid(20,0.1,0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)

        # test 3 
        self.x_pid = pid(1.0, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.y_pid = pid(1.0, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.z_pid = pid(1.2, 0.0,0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        
        self.vx_pid = pid(1.3, 0.2, 0.2, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vy_pid = pid(1.3, 0.2, 0.2, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vz_pid = pid(18.0, 0.1, 0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
        
        # ########################## TODO - SET GAINS HERE ##############################
        # ###############################################################################
        # # NED velocity controller. EDIT GAINS HERE
        # self.vx_pid = pid(0, 0.0, 0.0, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(0, 0.0, 0.0, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vz_pid = pid(0, 0, 0.0, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
        ##############################################################################
        ##############################################################################
        ...
        # self.x_pid = pid(1.0, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(1.0, 0.0, 0.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.z_pid = pid(0.1, 0.0,0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        
        # self.vx_pid = pid(0.5, 0.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vy_pid = pid(0.5, 0.0, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        # self.vz_pid = pid(10.0, 0.1, 0.1, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
    ...
    def step(self, X, WP, VEL_SP, ACC_SP):
        ...
        return U# return f_inertial for tuning purposes
...
class QuadrotorController:
    ...
    def get_desired_state(self, t):
        ...
        print (f"Velocities desired{vel_des}")
    ...
    def compute_control(self, current_state, t):
        ...
        return control_input, pos_des, vel_des
