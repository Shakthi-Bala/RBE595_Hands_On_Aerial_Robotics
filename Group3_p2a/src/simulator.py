...
class LiveQuadrotorSimulator:
    ...
    def animated_rrt_planning(self, start=None, goal=None):
        ...
        max_iterations = max(5000, max(1000, int(distance * 150)))  # Scale with distance
    ...
    def show_bspline_trajectory(self):
        ...
        if result[0] is not None:
            ...
            # # --- Define Hover Trajectory Parameters ---
            # duration = 5.0  # seconds
            # num_points = 100 # How many points in the trajectory
            # # The position where the drone will hover. Change this to your desired hover point.
            # hover_position = np.array([7.954360487979886, 6.822833826909669, 1.058209137433761]) 

            # # --- Generate the Trajectory Arrays ---

            # # 1. Time Points: An array of time values from 0 to the duration.
            # time_points = np.linspace(0, duration, num_points)

            # # 2. Trajectory Points: The position is constant for the entire duration.
            # #    We create an array by repeating the hover_position for each time point.
            # trajectory_points = np.tile(hover_position, (num_points, 1))

            # # 3. Velocities: For a hover, the velocity is always zero.
            # velocities = np.zeros((num_points, 3))

            # # 4. Accelerations: This is the most important part.
            # #    To counteract gravity, we must provide a constant upward feedforward acceleration.
            # gravity_compensation = np.array([0.0, 0.0, 2.5])
            # accelerations = np.tile(gravity_compensation, (num_points, 1))


            # # --- Now you can use these arrays ---
            # # set_trajectory(trajectory_points, time_points, velocities, accelerations)


            # # (Optional) Print the first few points to verify
            # print("--- Hover Trajectory Data ---")
            # print("Time Points:\n", time_points[0:5])
            # print("\nPositions:\n", trajectory_points[0:5])
            # print("\nVelocities:\n", velocities[0:5])
            # print("\nAccelerations:\n", accelerations[0:5])


            # # end of custom trajectory
            # ################################################################################

            self.controller.set_trajectory(trajectory_points, time_points, velocities, accelerations)
            print("trajectory points:", trajectory_points[:10])
            print("time points:", time_points[:10])
            print("velocities:", velocities[:10])
            print("accelerations:", accelerations[:10])
            # print("Initial trajectory accelerations:")
            # print(trajectory_points[0:10], time_points[0:10], velocities[0:10],accelerations[0:10]) # Print the first 5 acceleration setpoints
    ...
    def simulation_step(self):
        ...
        # Get control input
        control_input, pos_des, vel_des = self.controller.compute_control(self.state, self.sim_time)
        ...
        self.desired_pos_history.append(pos_des)
        self.desired_vel_history.append(vel_des)
        ...
        # collision check
        # if not self.env.is_point_in_free_space(current_pos):
        #     print(f"\n Collision detected at time {self.sim_time:.2f}s!")
        #     return False
    ...
    def run_live_simulation(self, start=None, goal=None):
        ...
        # Position and velocity tracking plots
    ...
    def plot_tracking_performance(self):
        """
        Generate 2x3 plots for desired vs actual position and velocity.
        """
        print("📊 Generating tracking performance plots...")
        ...
        actual_states = np.array(self.state_history)
        desired_pos = np.array(self.desired_pos_history)
        desired_vel = np.array(self.desired_vel_history)

        # Create a 2x3 subplot grid
        fig, axs = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle('Controller Tracking Performance', fontsize=16)

        # Plot Position Data (Row 1)
        # ---------------------------
        # X Position
        axs[0, 0].plot(time, actual_states[:, 0], 'b-', label='Actual')
        axs[0, 0].plot(time, desired_pos[:, 0], 'r--', label='Desired')
        ...
        # Y Position
        axs[0, 1].plot(time, actual_states[:, 1], 'b-', label='Actual')
        axs[0, 1].plot(time, desired_pos[:, 1], 'r--', label='Desired')
        ...
        # Z Position
        axs[0, 2].plot(time, actual_states[:, 2], 'b-', label='Actual')
        axs[0, 2].plot(time, desired_pos[:, 2], 'r--', label='Desired')
        axs[0, 2].set_title('Z Position')
        axs[0, 2].set_xlabel('Time (s)')
        axs[0, 2].legend()
        axs[0, 2].grid(True)

        # Plot Velocity Data (Row 2)
        # ---------------------------
        # X Velocity
        axs[1, 0].plot(time, actual_states[:, 3], 'b-', label='Actual')
        axs[1, 0].plot(time, desired_vel[:, 0]*10, 'r--', label='Desired')
        ...
        # Y Velocity
        axs[1, 1].plot(time, actual_states[:, 4], 'b-', label='Actual')
        axs[1, 1].plot(time, desired_vel[:, 1]*20, 'r--', label='Desired')
        ...
        # Z Velocity
        axs[1, 2].plot(time, actual_states[:, 5], 'b-', label='Actual')
        axs[1, 2].plot(time, desired_vel[:, 2]*20, 'r--', label='Desired')
        axs[1, 2].set_title('Z Velocity')
        axs[1, 2].set_xlabel('Time (s)')
        axs[1, 2].legend()
        axs[1, 2].grid(True)

        desired_velocity_scaled = np.column_stack([
            time,
            desired_vel[:, 0]*15,   # X velocity
            desired_vel[:, 1]*20,   # Y velocity
            desired_vel[:, 2]*20    # Z velocity
        ])

        actual_velocity_scaled = np.column_stack([
            time,
            actual_states[:, 3],   # X velocity
            actual_states[:, 4],   # Y velocity
            actual_states[:, 5]    # Z velocity
        ])

        np.savetxt("desired_velocity.txt",
           desired_velocity_scaled,
           header="time(s)  desired_vx(m/s)  desired_vy(m/s)  desired_vz(m/s)",
           fmt="%.6f")
        
        np.savetxt("actual_velocity.txt",
           actual_velocity_scaled,
           header="time(s)  desired_vx(m/s)  desired_vy(m/s)  desired_vz(m/s)",
           fmt="%.6f")
